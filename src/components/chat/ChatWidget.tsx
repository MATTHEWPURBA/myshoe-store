...
const ChatWidget: React.FC = () => {
  ...
  const { messages, sendMessage, isConnected, isTyping, error } = useChat();
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
      {/* Chat button */}
      <button aria-label={isOpen ? 'Close chat' : 'Open chat'} className="bg-blue-600 text-white p-4 rounded-full shadow-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500" onClick={toggleChat}>
        {isOpen ? (
      ...
      {isOpen && (
        <div className="absolute bottom-16 right-0 w-80 sm:w-96 bg-white rounded-lg shadow-xl flex flex-col h-96 border border-gray-200">
                ...
                {recommendedProducts.length > 0 && (
                  <div className="mt-4 space-y-2">
                    ...
                    {recommendedProducts.map((product) => (
                      <ProductRecommendation key={product.id} product={product} onClick={() => handleProductSelect(product.id)} />
                    ))}
                  </div>
                )}
          ...
          <form onSubmit={handleSend} className="border-t border-gray-200 p-4">
            <div className="flex">
              <input type="text" ref={inputRef} value={input} onChange={(e) => setInput(e.target.value)} placeholder="Type your message..." className="flex-1 border border-gray-300 rounded-l-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" disabled={!isConnected} />
              <button type="submit" className="bg-blue-600 text-white px-4 py-2 rounded-r-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-400" disabled={!isConnected || !input.trim()}>
